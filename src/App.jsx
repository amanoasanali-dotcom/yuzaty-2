...
function App() {
  ...
  const [submitCount, setSubmitCount] = useState(0) // Track how many times they submitted
    ...
    <div className="min-h-screen bg-black flex justify-center">
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none flex justify-center">
        <div className="relative w-full max-w-[500px] h-full overflow-hidden">
          <video ref={videoRef} key={videos[currentVideoIndex]} autoPlay muted playsInline onEnded={handleVideoEnd} className="absolute min-w-full min-h-full object-cover opacity-60 scale-105">
        ...
        {isStarted && (
          <>
            <section className="h-screen flex flex-col items-center justify-center p-6 text-center">
              <h1 className="intro-title text-4xl md:text-5xl font-black text-white tracking-tighter drop-shadow-2xl mb-4 leading-tight">
                Сізді Қыз Ұзату<br />тойына шақырамыз
              </h1>
              <p className="intro-subtitle text-xl text-white/90 mb-2 font-light drop-shadow-lg">
                Аделияға арналған арманша кеш
              </p>
              <p className="text-base text-white/70 mb-12 max-w-[280px] drop-shadow-lg">
                Ресторан Fiesta Hall<br />
                1 тамыз 2026 ж.
              </p>
            ...
            <section className="py-20 px-6 flex flex-col gap-12 bg-gradient-to-b from-transparent via-black/80 to-black/40 backdrop-blur-sm">
              ...
              {hallPhotos.map((photo, i) => (
                <div key={i} className="hall-photo-item w-full aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl border border-white/10">
                  <img src={photo} alt={`Hall ${i + 1}`} className="w-full h-full object-cover" />
                </div>
              ))}
            </section>
              ...
              <div className="rsvp-section-content relative z-10 w-full flex flex-col items-center">
                ...
                {step === 'success' && (
                  <div className="success-message text-center">
                    ...
                    {/* Only show Repeat button if it's the first time */}
                    {submitCount < 2 && (
